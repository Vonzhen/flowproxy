// --- [ FlowProxy | Server 服务端视图模块 | v1.0 Contract Aligned Edition ] ---
// 功能：渲染 FlowProxy 作为入站代理服务端 (Inbounds) 的控制面
// 核心升级：废除旧版 service.list 轮询，强制接入 flowproxy.system status 唯一真相链
...
// ⭐ 斩首：废除 callServiceList，对接 Truth Chain
...
const CBIGenValue = form.Value.extend({
    ...
    renderWidget(/* ... */) {
        ...
        (node.querySelector('.control-group') || node).appendChild(E('button', {
            ...
            title: _('Generate'),
            click: ui.createHandlerFn(this, handleGenKey, this.fp_options || this.option)
        }, [ _('Generate') ]));
...
// ⭐ 对齐真相引擎：只读取快照中的 process.running
function getServiceStatus() {
    return L.resolveDefault(callSystemStatus(), {}).then((res) => {
        return (res && res.process) ? res.process.running : false;
    });
}
...
function renderStatus(isRunning, version) {
    ...
    let renderHTML;
    if (isRunning)
        renderHTML = spanTemp.format('green', _('FlowProxy Server'), version, _('RUNNING'));
    else
        renderHTML = spanTemp.format('red', _('FlowProxy Server'), version, _('NOT RUNNING'));
    ...
    if (typeof option === 'object') {
        ...
        switch (required_method) {
            case 'none':
                ...
                break;
            case 'uuid':
                password = fp.generateRand('uuid');
                break;
            default:
                password = fp.generateRand('hex', 16);
                break;
        }
        /* AEAD */
        ((length) => {
            if (length && length > 0)
                password = fp.generateRand('base64', length);
        })(fp.shadowsocks_encrypt_length[required_method]);
...
return view.extend({
    ...
    render(data) {
        ...
        m = new form.Map('flowproxy', _('FlowProxy Server'),
            _('The modern proxy platform. Driven by declarative contracts.'));
        ...
        s = m.section(form.NamedSection, 'server', 'flowproxy', _('Global settings'));
        ...
        s.modaltitle = L.bind(fp.loadModalTitle, this, _('Server'), _('Add a server'), data[0]);
        s.sectiontitle = L.bind(fp.loadDefaultLabel, this, data[0]);
        s.renderSectionAdd = L.bind(fp.renderSectionAdd, this, s);

        o = s.option(form.Value, 'label', _('Label'));
        o.load = L.bind(fp.loadDefaultLabel, this, data[0]);
        o.validate = L.bind(fp.validateUniqueValue, this, data[0], 'server', 'label');
        o.rmempty = false;
        o.modalonly = true;
        ...
        o.datatype = 'port';
        o.validate = L.bind(fp.validateUniqueValue, this, data[0], 'server', 'port');
                ...
                if (required_type.includes(type)) {
                    if (type === 'shadowsocks') {
                        ...
                        if (encmode === 'none')
                            ...
                            return fp.validateBase64Key(24, section_id, value);
                        else if (['2022-blake3-aes-256-gcm', '2022-blake3-chacha20-poly1305'].includes(encmode))
                            return fp.validateBase64Key(44, section_id, value);
                    }
        ...
        o.onchange = function(ev, section_id, value) {
            ...
            let tls_element = this.map.findElement('id', 'cbid.flowproxy.%s.tls'.format(section_id)).firstElementChild;
        ...
        for (let i of fp.tls_cipher_suites)
            o.value(i);
        o.depends('tls', '1');
        o.optional = true;
        o.modalonly = true;
        ...
        o.value('/etc/flowproxy/certs/server_publickey.pem');
        ...
        o.depends({'tls': '1', 'tls_cert_path': '/etc/flowproxy/certs/server_publickey.pem'});
        o.onclick = L.bind(fp.uploadCertificate, this, _('certificate'), 'server_publickey');
        o.modalonly = true;

        o = s.option(form.Value, 'tls_key_path', _('Key path'),
            _('The server private key, in PEM format.'));
        o.value('/etc/flowproxy/certs/server_privatekey.pem');
        o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': '0'});
        o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': null});
        o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': '0'});
        o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': null});
        ...
        o.inputstyle = 'action';
        o.inputtitle = _('Upload...');
        o.depends({'tls': '1', 'tls_key_path': '/etc/flowproxy/certs/server_privatekey.pem'});
        o.onclick = L.bind(fp.uploadCertificate, this, _('private key'), 'server_privatekey');
        o.modalonly = true;
            ...
            node.appendChild(E('div',  { 'class': 'control-group' }, [
                ...
                E('button', {
                    ...
                    click: ui.createHandlerFn(this, () => {
                        this.fp_options.params = document.getElementById(cbid).value;

                        return handleGenKey.call(this, this.fp_options);
                    })
                }, [ _('Generate') ])
            ]));
