...
function setup(trace_id) {
    try {
        ...
        if (has_tproxy) {
            ...
            ExecSafe(BIN.IP, ['rule', 'add', 'fwmark', tproxy_mark, 'table', tproxy_mark], null, trace_id);
            ExecSafe(BIN.IP, ['route', 'add', 'local', '0.0.0.0/0', 'dev', 'lo', 'table', tproxy_mark], null, trace_id);
            
            if (ipv6_support) {
                ExecSafe(BIN.IP, ['-6', 'rule', 'add', 'fwmark', tproxy_mark, 'table', tproxy_mark], null, trace_id);
                ExecSafe(BIN.IP, ['-6', 'route', 'add', 'local', '::/0', 'dev', 'lo', 'table', tproxy_mark], null, trace_id);
            }
        }
