/**
 * FlowProxy | system/network/firewall_tproxy.uc | v2.0
 * Role: Redirect+TProxy nftables ruleset compiler.
 */
...
function build_tproxy_firewall(trace_id) {
    try {
        log(trace_id, 'INFO', 'FIREWALL_TPROXY', 'Compiling Redirect+TProxy nftables ruleset...');
        ...
        push(rules, sprintf("table %s {", DATAPLANE.NFT_TABLE));
        ...
        push(rules, sprintf("        meta mark { %s, 255 } counter return", self_mark));
        ...
        writefile(PATH.FIREWALL_NFT, join('\n', rules));
        log(trace_id, 'INFO', 'FIREWALL_TPROXY', 'Compiled ruleset to ' + PATH.FIREWALL_NFT);
        return Success(true, 200, trace_id);
    } catch(e) {
        let err_msg = "" + e;
        log(trace_id, 'CRIT', 'FIREWALL_TPROXY', 'Compilation Exception: ' + err_msg);
        return Fail(ERR.E_SYSTEM_BUSY, "Firewall Build Exception: " + err_msg, trace_id);
    }
}

export { build_tproxy_firewall };
